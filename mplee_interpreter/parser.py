...
class Parser:
    ...
    def atom(self):
        ...
        if tok.type in (TT_INT, TT_FLOAT):
            ...
            if self.tokens[new_token_idx + 1].type == TT_LSQUARE:
                next_tokens = [
                    self.tokens[new_token_idx + 1],
                    self.tokens[new_token_idx + 2],
                    self.tokens[new_token_idx + 3],
                ]
                if (
                    next_tokens[0].type == TT_LSQUARE
                    and next_tokens[1].type == TT_INT
                    and next_tokens[2].type == TT_RSQUARE
                ):
                    listres = self.list_index_pointer_expr(
                        self.tokens, new_token_idx, save_token, next_tokens, res
                    )
    ...
    def list_index_pointer_expr(
        self, tokens, token_position_idx, save_token, next_tokens, res
    ):
        res.register_advancement()
        res.register_advancement()
        res.register_advancement()
        ...
        List_index_node = ListIndexPointerNode(
            VarAccessNode(save_token), NumberNode(next_tokens[1])
        )
        if tokens[token_position_idx + 4].type == TT_EQ:
            ...
            if tokens[token_position_idx + 5].type in (TT_INT, TT_STRING, TT_FLOAT):
                return BinOpNode(
                    List_index_node,
                    tokens[token_position_idx + 4],
                    NumberNode(tokens[token_position_idx + 5]),
                )
            else:
